#!/usr/bin/env python3
"""Convert NotoSansMono 13px font_to_py hmap font to russhughes write() format.
Input:  /tmp/mono13raw.py  (generated by font_to_py)
Output: app/fonts/mono13.py"""
import sys, os
sys.path.insert(0, '/tmp')
import mono13raw as src

HEIGHT = src.height()
CHARS = ''.join(chr(c) for c in range(0x20, 0x7F))   # ASCII printable only
...
out = f'''# NotoSansMono-Regular 13px — monospaced, ASCII 0x20-0x7e
MAP = {repr(map_str)}
BPP = 1
HEIGHT = {HEIGHT}
MAX_WIDTH = {max_width}
...
out_path = os.path.join(os.path.dirname(__file__), '..', 'app', 'fonts', 'mono13.py')
