...
Reads app/fonts/dejavu14.py, writes app/fonts/dejavu14_ru.py"""
import sys, os
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', 'app', 'fonts'))
import dejavu14 as src

HEIGHT = src.height()
_codepoints = (
    list(range(0x20, 0x7F)) +   # ASCII printable
    list(range(0xA0, 0x100)) +  # Latin-1 supplement (°, £, ², ©, ×, etc.)
    [0x2013, 0x2014,            # en dash, em dash
     0x2018, 0x2019,            # left/right single quotes
     0x201C, 0x201D,            # left/right double quotes
...
def extract_bits(glyph, w, h):
    """Extract pixels from font_to_py hmap (row-padded) as a flat bit list."""
    bits = []
    row_bytes = (w + 7) // 8
    for row in range(h):
        for col in range(w):
...
# Choose offset_width: 2 bytes for offsets < 65536 bits
OFFSET_WIDTH = 2

def fmt_bytes(data, name, indent=4):
    lines = []
    row = []
...
# Build OFFSETS as bytes (2 bytes per entry, big-endian)
offsets_bytes = bytearray()
for o in offsets:
    offsets_bytes.append((o >> 8) & 0xFF)
    offsets_bytes.append(o & 0xFF)

out = f'''# Auto-converted from dejavu14.py (font_to_py) to russhughes write() format
# {HEIGHT}px DejaVu Sans Bold, ASCII 0x20-0x7e
MAP = {repr(map_str)}
BPP = 1
HEIGHT = {HEIGHT}
MAX_WIDTH = {max_width}
...
out_path = os.path.join(os.path.dirname(__file__), '..', 'app', 'fonts', 'dejavu14_ru.py')
